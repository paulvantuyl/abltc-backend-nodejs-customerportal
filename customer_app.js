...
const saltRounds = 5;
const password = "admin";                        // Example password to be hashed
...
mongoose.connect(uri, {'dbName': 'Coursera'});
...
app.get('/api/logout', async (req, res) => {
    req.session.destroy((err) => {
        if (err) {
            ...
            res.cookie('username', '', { expires: new Date(0) }); // Clear the username cookie
